...
from myblog.util import PageInfo
...
def category(request, name):
    ...
    return render(request, 'blog/category.html', {"blog_list": _blog_list, "page_info": page_info, "category": name})
...
def tag(request, name):
    ...
    return render(request, 'blog/tag.html', {"blog_list": _blog_list, "tag": name, "page_info": page_info})
...
def archive(request):
    ...
    data = sorted([{"date": _[0], "blogs": _[1]} for _ in archive_dict.items()], key=lambda item: item["date"], reverse=True)
...
def page_found_error(request, exception):
